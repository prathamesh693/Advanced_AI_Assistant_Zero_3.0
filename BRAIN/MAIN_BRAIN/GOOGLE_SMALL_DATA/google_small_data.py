...
# Set the path to you Chromedriver executable
...
def search_brain(text):
    try:
        ...
        # Extract and print the first 3-4 sentences from the first search result
        ...
        filtered_sentences = [sentence for sentence in sentences if not re.search(r'(https?://|\d{4}|www\.)', sentence)]

        # Join the remaining sentences
        result_text = ','.join(filtered_sentences[:10])
        result_text = result_text.replace("Featured snippet from the web", "Featured snippet from the web")
        return result_text
